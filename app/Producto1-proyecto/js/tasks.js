...
async function createTask(title, description, panelId, responsible) {
  // Añadir responsible como parámetro
  const mutation = `
        mutation {
...
async function displayTasks() {
  ...
  tasks.forEach((task) => {
    if (task.panelId === currentPanelId) {
      // Filtrar solo por el panelId correspondiente
    ...
    <div class="card-body">
        ...
        <p class="card-text"><strong>Responsable:</strong> ${
          task.responsible
        }</p>   <!-- Mostrar responsable -->
        <p class="card-text"><strong>Creado el:</strong> ${new Date(
          task.createdAt
        ).toLocaleString()}</p> <!-- Mostrar fecha y hora de creación -->
        <button class="btn btn-danger btn-sm" onclick="deleteTask('${
          task.id
        }')">Eliminar</button>
        <button class="btn btn-primary btn-sm" onclick='openEditModal(${JSON.stringify(
          task
        )})'>Editar</button>
    </div>
`;
    ...
    try {
      if (taskId) {
        ...
        if (!panelId) {
          ...
          alert(
            "Esto es una prueba ESTATICA DE HTML, para crear nuevas tareas, crear un nuevo proyecto! "
          );
